...
// 只需要首地址+偏移量即可。
void intc_setvectaddr(unsigned long intnum, void (*handler)(void))   //ARM32位中，long 和int 都是32位的
{
    //VIC0
    if(intnum<32)
    {
        *( (volatile unsigned long *)(VIC0VECTADDR + 4*(intnum-0)) ) = (unsigned)handler; //简化为unsigned
    }
    //VIC1
    else if(intnum<64)
    {
