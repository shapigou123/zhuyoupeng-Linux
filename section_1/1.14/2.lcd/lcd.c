...
// FB地址
#define FB_ADDR			(0x23000000)     //内存的合理位置即可！！对齐的多一点（可以稍微大点，比如1M对齐）
#define ROW				(600)
#define COL				(1024)
#define HOZVAL			(COL-1)
...
#define RightBotY   599
	...
	//一个像素用三个字节表示，RGB888。但是为了字节对齐 所以乘了4 虽然浪费了一定空间 但速率更高！！

	
	// 使能channel 0和2（window0 2）传输数据
	SHADOWCON = 0x5;

}

// 在像素点(x, y)处填充为color颜色  x（0,1024） y（0,599）
