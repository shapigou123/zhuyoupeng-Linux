...
// count记录当前正在绘制的像素的次序
    ...
    for (j=y; j<(y+16); j++)  //从行开始画，所以第一维是Y。写字的时候得移动坐标 所以是（y+16）、（x+8）
    {  
        for (i=x; i<(x+8); i++)  //精确到某一列 即具体到某一个像素点了
        {  
            if (i<XSIZE && j<YSIZE)  
            {  
			// 在坐标(i, j)这个像素（8位的每一位）处判断是0还是1，如果是1写color；如果是0直接跳过
...
void draw_ascii_ok(unsigned int x, unsigned int y, unsigned int color, unsigned char *str)
{
    ...
    {  
        ...
        show_8_16(x, y, color, ch); //（把字符指针传到这个函数里面来显示）
...
void lcd_test(void)
{
	...
	draw_ascii_ok(0, 0, RED, "大家BCDabcd1234!@#$%");
	// 测试画斜线
	//lcd_draw_background(WHITE);			// 如果没有加这一行，背景色是花花绿绿的
	glib_line(0, 599, 1023, 0, RED);     //左下角到右下角
	glib_line(0, 0, 1023, 599, RED);
	draw_circular(511, 299, 50, GREEN);

	
	
/*	// 测试画横线、竖线
	lcd_draw_background(WHITE);
	lcd_draw_hline(350, 450, 300, RED);
