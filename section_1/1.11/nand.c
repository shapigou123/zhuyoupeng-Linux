...
{
	unsigned long i;
	while( !(rNFSTAT & (1<<4)) )        // NFSTAT为0 一直循环  ；1表示擦完！
	for(i=0; i<10; i++);
}
...
int nand_block_erase(unsigned long block_num)
{
	...
	for(i=0; i<10; i++);
	// 3. 等待就绪  
	nand_wait_idle();      // NFSTAT为0 一直循环  ；1表示擦完！
...
int nand_page_read(unsigned int pgaddr, unsigned char *buf, unsigned int length)
{
	...
	for (i=0; (i<NAND_PAGE_SIZE) && (length!=0); i++,length--)  //length 为要读取的长度
		*buf++ = nand_read8();
...
int nand_page_read32(unsigned int pgaddr, unsigned int *buf, unsigned int lengthB) //连续读取32位
{
	int i = 0;
