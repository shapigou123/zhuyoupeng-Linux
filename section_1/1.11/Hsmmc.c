...
int Hsmmc_Init(void)
{
	...
	Timeout = 1000; // Wait max 10 ms
	while (__REGb(HSMMC_BASE+SWRST_OFFSET) & (1<<0)) {   //10ms之后 bit0若还是1 则复位失败！
		if (Timeout == 0) {
	...
	if (Hsmmc_IssueCommand(CMD8, 0x1aa, 0, CMD_RESP_R7)) { // 成立则 没有回复,MMC/sd v1.x/not card
			for (i=0; i<100; i++) {
				// CMD55 + CMD41 = ACMD41
	...
	if (CardType == SD_HC || CardType == SD_V1 || CardType == SD_V2) {
		...
		Hsmmc_Get_SCR(&SCR);   //识别卡的版本号之后 再设置合适的时钟
		...
		if (!Hsmmc_SetBusWidth(4)) {  //4总线工作模式
