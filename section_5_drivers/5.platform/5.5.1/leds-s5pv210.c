...
struct s5pv210_gpio_led {
	struct led_classdev		 cdev;   //s5pv210_led_set传参只能传该类型的结构体指针；
	struct s5pv210_led_platdata	*pdata; //利用里面的pdata->gpio来区分不同的GPIO
};
			    ...
			    enum led_brightness value)
{
	...
	//struct led_classdev *反推得到struct s5pv210_gpio_led *
...
static int s5pv210_led_probe(struct platform_device *dev)
{
	...
	//确保每次匹配时传过来的platform_data（LED1 2 3 4）都不一样
...
{
	struct s5pv210_gpio_led *p = pdev_to_gpio(dev); //由struct platform_device * 得到s5pv210_gpio_led *
