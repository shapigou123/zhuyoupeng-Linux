...
#define SERPORT		9003
#define SERADDR		"192.168.10.20"		// ifconfig看到的
#define BACKLOG		100
...
int main(void)
{
	...
	// 建立连接之后就可以通信了
	// 客户端给服务器发
	ret = recv(clifd, recvbuf, sizeof(recvbuf), 0);  //用sizeof(recvbuf) 怕真正接受的长度比给定的要大
	printf("成功接收了%d个字节\n", ret);
	...
	while (1)
	{
		...
		//printf("成功接收了%d个字节\n", ret);
		printf("client发送过来的内容是：%s\n", recvbuf);	
		memset(recvbuf, 0, sizeof(recvbuf));    //recv是循环使用的，每次使用完要清除它
	}
#endif
